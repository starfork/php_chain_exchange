 ...
 * @param int|null $weekIndex    周索引
 * @return array               array,返回起始时间戳和结束时间戳组成的数组
 */
 ...
 * @param string $start    在字符串上开始截取的位置（从0开始的index）
 * @param string $len      在字符串上截取的长度
 ...
 * @return string:   返回文件的后缀名，若文件不存在返回false
 */
 ...
 * @param string $fileTempName 给下载的文件一个临时名字
 */
 ...
 * @param string $fileName 保存位置绝对路径文件名
 ...
 * @param number $pageIndex        当前页的页码
 * @param number $pageSize         每页数量
 * @param number $LRNum            当前页的左右分别允许显示的页码个数最大值
 * @param string $action           访问路径：/xxx/
 * @return array              返回数组：包含当前页显示页码的起始页码和结束页码以及左右是否显示省略号
 */
 ...
 * @return boolean|string|array <boolean, string>   array，返回目录结构组成的数组
 */
 ...
 * @param string|array $dirArr   获取到的目录结构数组
...
/**
 * 兼容中文路径的basename()
 * @param string $fileName 文件系统的路径
 * @return string            string 返回路径中的文件名部分
 */
 ...
 * @param string $source   需要复制的源路径，如果是一个目录的话，会将连带着该目录一起复制到目标目录中
 * @param string $dest     需要复制到的目标路径
